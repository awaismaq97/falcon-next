...
The extraction itself lives in ``falcon.text_extract``, because Drive downloads
need exactly the same parsing and a domain module cannot reasonably import a
router to get it. This module is the HTTP shell around it: upload limits, the
identity check, storage, and mapping an extraction failure onto a status code.
"""
...
from falcon.text_extract import UnsupportedDocument, ext_of, extract
...
async def extract_document(
    ...
    try:
        text = extract(name, data, ext_of(name))
    except UnsupportedDocument as exc:
        # The format is the problem, not the file — 415, with the reason as
        # written (it names the fix for a legacy .doc/.xls).
